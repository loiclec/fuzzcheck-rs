...
pub fn options_parser() -> Options {
    ...
    options
        ...
        .optflag("", NO_ARTIFACTS_FLAG, 
            format!("do not save artifacts, overrides --{artifacts}", artifacts=ARTIFACTS_FLAG).as_str()
        )
        .optopt("", INPUT_FILE_FLAG, "file containing a JSON-encoded input", "PATH")        
        .optopt("", CORPUS_SIZE_FLAG, 
            format!("target size of the corpus (default: {default})", default=DEFAULT_CORPUS_SIZE).as_str(),
            "N"
        )
        .optopt("", MAX_INPUT_CPLX_FLAG, 
            format!("maximum allowed complexity of inputs (default: {default})", default=DEFAULT_MAX_INPUT_CPLX).as_str(),
            "N"
        )
        .optopt("", MUT_DEPTH_FLAG, 
            format!("number of times inputs are mutated each iteration (default: {default})", default=DEFAULT_MUT_DEPTH).as_str(),
            "N"
        )
...
The command {c} is not supported. It can either be ‘{fuzz}’, ‘{tmin}’, or ‘{cmin}’.
        ...
        let max_input_cplx: f64 = matches.opt_str(MAX_INPUT_CPLX_FLAG)
            .map(|x| x.parse::<usize>().ok())
            .flatten()
            .unwrap_or(DEFAULT_MAX_INPUT_CPLX) as f64;

        let input_file: Option<PathBuf> = matches.opt_str(INPUT_FILE_FLAG)
            .map(|x| x.parse::<PathBuf>().ok())
            .flatten();

        let corpus_size: usize = matches.opt_str(CORPUS_SIZE_FLAG)
            .map(|x| x.parse::<usize>().ok())
            .flatten()
        ...
        let artifacts_folder: Option<PathBuf> = 
            if !matches.opt_present(NO_ARTIFACTS_FLAG) { 
                matches.opt_str(ARTIFACTS_FLAG)
        ...
        let max_nbr_of_runs: usize = matches.opt_str(MAX_NBR_RUNS_FLAG)
            .map(|x| x.parse::<usize>().ok())
            .flatten()
            .unwrap_or(core::usize::MAX);

        let mutate_depth: usize = matches.opt_str(MUT_DEPTH_FLAG)
            .map(|x| x.parse::<usize>().ok())
            .flatten()
