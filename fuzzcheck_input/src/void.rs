...
impl InputGenerator for VoidGenerator {
    ...
    fn complexity(_input: &Self::Input) -> f64 { 0.0 }

    fn hash<H>(_input: &Self::Input, _state: &mut H) where H: Hasher { }
    ...
    fn from_data(_data: &Vec<u8>) -> Option<Self::Input> { Some(()) }
    fn to_data(_input: &Self::Input) -> Vec<u8> { vec![] }
}