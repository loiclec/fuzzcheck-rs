    ...
    Map: Fn(&T) -> U,
{
    ...
    type UnmutateToken = Mut::UnmutateToken;
    ...
    fn ordered_arbitrary(&self, step: &mut Self::ArbitraryStep, max_cplx: f64) -> Option<(U, Self::Cache, Self::MutationStep)> {
        self.mutator.ordered_arbitrary(step, max_cplx).map(|(value, cache, step)| {
            let cache = Cache {
                value,
                cache,
            };
