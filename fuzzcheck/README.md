...
mutator if `A`, `Vec<B>`, and `C` all implement `HasDefaultMutator`. For now, only mutators for structs whose fields do not use private types can be automatically generated.
...
incompatible versions of the types and traits defined by `A`. To avoid this situation, you can 
