...
pub fn fuzzcheck_make_mutator(
    ...
    if let Some(first_ident) = attr_parser.eat_any_ident() {
        if first_ident == ident!("name") {
            if let Some(_) = attr_parser.eat_punct('=') {
                if let Some(name) = attr_parser.eat_any_ident() {
                    if let Some(s) = parser.eat_struct() {
                        derive_struct_mutator(s, name, false, &mut tb);
                    } else if let Some(e) = parser.eat_enumeration() {
                        derive_enum_mutator(e, name, false, &mut tb)
                    } else {
                        extend_ts!(&mut tb,
                            "compile_error ! ("
                    ...
                    extend_ts!(&mut tb,
                        "compile_error ! ("
                        Literal::string(&format!("fuzzcheck_mutators_derive: expected an identifier  after setting: \"{}=\"", first_ident))
                        ") ;"
                    );
                }
            } else {
                extend_ts!(&mut tb,
                    "compile_error ! ("
                    Literal::string(&format!("fuzzcheck_mutators_derive: expected `=` after setting: \"{}\"", first_ident))
                    ") ;"
                );
            }
        } else {
            extend_ts!(&mut tb,
                "compile_error ! ("
                Literal::string(&format!("fuzzcheck_mutators_derive: unrecognized setting \"{}\"", first_ident))
                ") ;"
            );
        }
    }
...
pub fn fuzzcheck_derive_mutator(
    ...
    if let Some(s) = parser.eat_struct() {
        let main_mutator_name = ident!(s.ident "Mutator");
        derive_struct_mutator(s, main_mutator_name, derive_default, &mut tb);
    } else if let Some(e) = parser.eat_enumeration() {
        let main_mutator_name = ident!(e.ident "Mutator");
        derive_enum_mutator(e, main_mutator_name, derive_default, &mut tb)
    } else {
        extend_ts!(&mut tb,
            "compile_error ! ("
            Literal::string("fuzzcheck_mutators_derive could not parse the structure")
...
fn derive_struct_mutator(parsed_struct: Struct, main_mutator_name: Ident, derive_default: bool, tb: &mut TokenBuilder) {
    /*
    ...
    */
    ...
    if !parsed_struct.struct_fields.is_empty() {
        derive_struct_mutator_with_fields(&parsed_struct, main_mutator_name, derive_default, tb)
    } else {
        derive_unit_mutator(&parsed_struct, main_mutator_name, derive_default, tb);
    }
...
fn derive_struct_mutator_with_fields(
    parsed_struct: &Struct,
    main_mutator_name: Ident,
    derive_default: bool,
    tb: &mut TokenBuilder,
) {
...
fn derive_enum_mutator(parsed_enum: Enum, main_mutator_name: Ident, derive_default: bool, tb: &mut TokenBuilder) {
    /*
        We put the whole implementation into a separate module and then re-export
        only the mutator.
    */
    ...
    if !parsed_enum.items.is_empty() {
        derive_enum_mutator_with_items(&parsed_enum, main_mutator_name, derive_default, tb)
    } else {
...
fn derive_enum_mutator_with_items(
    parsed_enum: &Enum,
    main_mutator_name: Ident,
    derive_default: bool,
    tb: &mut TokenBuilder,
...
fn derive_unit_mutator(parsed_struct: &Struct, main_mutator_name: Ident, derive_default: bool, tb: &mut TokenBuilder) {
    let default_mutator = ts!("fuzzcheck_mutators :: DefaultMutator");

    if derive_default {
        let generics_without_bounds = parsed_struct.generics.clone().removing_bounds_and_eq_type();
        let mutator_ident = main_mutator_name;
