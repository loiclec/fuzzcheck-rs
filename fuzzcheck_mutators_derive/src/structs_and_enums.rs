...
pub(crate) fn make_mutator_type_and_impl(params: CreateWrapperMutatorParams) -> TokenStream {
    ...
    let NameMutatorRecursingPartIndex = ident!(NameMutator "RecursingPartIndex");
...
Generated by a procedural macro of [`fuzzcheck`]",
    ...
    helper_type("RecursingPartIndex", true)
    ...
    "impl " NameMutator_generics cm.Default "for" NameMutator NameMutator_generics.removing_bounds_and_eq_type()
            ...
            for " NameMutator NameMutator_generics.removing_bounds_and_eq_type() NameMutator_where_clause "
        {
            ...
            #[doc(hidden)]
            type RecursingPartIndex =" NameMutatorRecursingPartIndex NameMutator_generics.removing_bounds_and_eq_type() ";
            #[doc(hidden)]
            ...
            fn default_recursing_part_index(&self, value: & " type_ident type_generics.removing_bounds_and_eq_type() ", cache: &Self::Cache) -> Self::RecursingPartIndex {
                Self::RecursingPartIndex::new(" InnerMutator_as_Mutator "::default_recursing_part_index(&self.mutator, value, &cache.inner))
            }
            #[doc(hidden)]
            #[no_coverage]
            fn recursing_part<'a, ___V, ___N>(&self, parent: &___N, value: &'a " type_ident type_generics.removing_bounds_and_eq_type() ", index: &mut Self::RecursingPartIndex) -> " cm.Option "<&'a ___V>
            where
                ___V: " cm.Clone " + 'static,
                ___N: " cm.fuzzcheck_traits_Mutator "<___V>,
            {
                " InnerMutator_as_Mutator "::recursing_part::<___V, ___N>(&self.mutator, parent, value, &mut index.inner)
            }
            #[doc(hidden)]
            #[no_coverage]
            fn lens<'a>(&self, value: &'a " type_ident type_generics.removing_bounds_and_eq_type() ", cache: &Self::Cache, path: &Self::LensPath) -> &'a dyn " cm.Any " {
                " InnerMutator_as_Mutator "::lens(&self.mutator, value, &cache.inner, &path.inner)
            }
            #[doc(hidden)]
            #[no_coverage]
            fn all_paths<'a>(&self, value: &'a" type_ident type_generics.removing_bounds_and_eq_type() ", cache: &'a Self::Cache) -> " cm.HashMap "<" cm.TypeId ", " cm.Vec "<Self::LensPath>> {
                let subr = " InnerMutator_as_Mutator "::all_paths(&self.mutator, value, &cache.inner);
                let mut r: " cm.HashMap "<" cm.TypeId ", " cm.Vec "<Self::LensPath>> = <_>::default();
                for (typeid, subpaths) in subr {
                    r.insert(typeid, subpaths.into_iter().map(|p| {
                        Self::LensPath::new(p)
                    }).collect());
                }
                r
            ...
            fn crossover_arbitrary(
                ...
                max_crossover_cplx: f64,
                max_cplx: f64,
            ) -> " cm.CrossoverArbitraryResult "<" type_ident type_generics.removing_bounds_and_eq_type() "> {
                " InnerMutator_as_Mutator "::crossover_arbitrary(&self.mutator, subvalue_provider, max_crossover_cplx, max_cplx)
            }
        }"
        if settings.default {
