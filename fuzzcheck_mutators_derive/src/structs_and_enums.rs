    ...
    "impl " NameMutator_generics cm.Default "for" NameMutator NameMutator_generics.removing_bounds_and_eq_type()
            ...
            for " NameMutator NameMutator_generics.removing_bounds_and_eq_type() NameMutator_where_clause "
        {
            ...
            fn crossover_mutate(
                ...
                let (unmutate, cplx) = " InnerMutator_as_Mutator "::crossover_mutate(&self.mutator, value, &mut cache.inner, subvalue_provider, max_cplx);
                (Self::UnmutateToken::new(unmutate), cplx)
            }
        }"
        if settings.default {
