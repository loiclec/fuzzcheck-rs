...
impl TokenBuilder {
    ...
    pub fn lifetime(&mut self, ident: &str) -> &mut Self {
...
impl TokenParser {
    ...
    pub fn is_punct(&mut self, what: char) -> bool {
        ...
        if let Some(TokenTree::Punct(current)) = &self.current {
            if current.as_char() == what && (matches!(what, '<' | '>' | '\'') || current.spacing() == Spacing::Alone){
                return true
            }
            else {
                return false
    ...
    pub fn eat_type(&mut self) -> Option<TokenStream> {
        ...
        else if let Some(tys) = self.eat_all_types() {
    ...
    // My own additions, not makepad’s
    ...
    pub fn eat_visibility(&mut self) -> Option<TokenStream> {
        ...
        if self.eat_ident("pub") {
            tb.add("pub");
            if self.is_paren() {
                if let Some(tt) = self.eat_any_group() {
                    tb.extend(tt);
                }
            }
            Some(tb.end())
